...
export function useFaceDetection(videoRef, options) {
  ...
  const loadModels = useCallback(async () => {
    try {
      ...
      if (!faceApiLoaded && typeof window !== 'undefined') {
        const faceApiModule = await import('face-api.js');
        
        // Try different ways to access the module
        if (faceApiModule.nets) {
          faceapi = faceApiModule;
        } else if (faceApiModule.default && faceApiModule.default.nets) {
          faceapi = faceApiModule.default;
        } else {
          // Try importing individual exports
          const { nets, detectAllFaces, TinyFaceDetectorOptions } = faceApiModule;
          if (nets) {
            faceapi = faceApiModule;
          } else {
            console.error('Unable to find nets in face-api module:', Object.keys(faceApiModule));
            throw new Error('face-api.js module structure not recognized');
          }
        }
      ...
      if (!faceapi || !faceapi.nets) {
        throw new Error('Could not properly load face-api.js');
      }
